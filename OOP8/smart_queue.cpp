...
template<class T>
std::shared_ptr< SmartQueueItem<T> > SmartQueue<T>::GetHead() {
    std::lock_guard<std::recursive_mutex> lock(this->queueMutex);
    return head;
}

template<class T>
bool SmartQueue<T>::Empty() {
    std::lock_guard<std::recursive_mutex> lock(this->queueMutex);
...
void SmartQueue<T>::push(std::shared_ptr<T> figure) {
    ...
    std::shared_ptr< SmartQueueItem<T> > it(new SmartQueueItem<T>(figure,&queueMutex));
    if(this->Empty()){
        head=it;
        tail=it;
...
template<class T>
std::ostream& operator<<(std::ostream& out, SmartQueue<T>& queue) {
    std::lock_guard<std::recursive_mutex> lock(queue.queueMutex);
    if(queue.Empty()) {
        out << "Queue is Empty\n\n";
    } else {
...
template  std::ostream& operator<<(std::ostream& out, SmartQueue<Figure>& obj);